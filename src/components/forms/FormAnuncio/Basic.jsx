...
export default function Basic({ anuncioData, onClose, onCallback }) {
  ...
  const optionsAcompanhantes =
    dataAcompanhantes?.map(acompanhante => ({ label: `${acompanhante.id.toString()} - ${acompanhante.nome}`, value: acompanhante.id.toString() })) || []
        ...
        <Grid.Col span={{ base: 12, lg: 6 }}>
          <Stack>
            ...
            <Grid>
              <Grid.Col span={6}>
                <Fields.DateField
                  inputProps={{ ...form.getInputProps('data_inicio'), label: 'Data Início', disabled: isSubmitting, required: true }}
                />
              </Grid.Col>
              <Grid.Col span={6}>
                <Fields.DateField
                  inputProps={{ ...form.getInputProps('data_fim'), label: 'Data Fim', disabled: isSubmitting, required: true }}
                />
              </Grid.Col>
              <Grid.Col span={4}>
                <Fields.StateField
